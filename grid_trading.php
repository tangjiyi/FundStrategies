...
function get_grid_price($net_value, $growth_rate, $prev_grid_price = null)
{
    ...
    for ($i=0; $i < $len; $i++) { 
        if ($net_value > $price_range[$i]) {
            if ($prev_grid_price) {
                $index = array_search($prev_grid_price, $price_range);
                if ($growth_rate < 0 && $i-1-$index >= 1) {
                    $prices = array_slice($price_range, $index+1, $i-1-$index);
                }elseif ($growth_rate > 0 && $index-$i >= 1) {
                    $prices = array_slice($price_range, $i, $index-$i);
                    sort($prices);
                }
            }else{
